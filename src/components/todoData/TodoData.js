        ...
        <div className='relative'>
            {todo.length>0 ? todo.map((v, i) => {
                return (
                    ...
                    // className='-top-2 -right-1 bg-teal-100 px-2 rounded-r-lg shadow drop-shadow-xl text-sm text-red-500 font-bold flex items-center cursor-pointer hover:scale-105 ease-in-out duration-300'>{'x'}</div>
                    ...
                    >{'x'}</div>
                    {/* <input type="checkbox" className={`${v.mark ? 'disable:opacity-100' : ''}`} disabled={v.mark ? true : false} checked={v.mark} onChange={e=>onCheckBox(e,i,v)} /> */}
